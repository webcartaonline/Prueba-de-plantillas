...
/* El grupo es ahora un PANEL PLEGABLE. Aparece contraído: cerrado solo se
   ve su encabezado —título, alerta e imagen, exactamente igual que antes—
   más una flecha que invita a abrirlo; al pulsarlo se despliega la lista de
   platos. El plegado lo hace el propio navegador con <details>/<summary>,
   así que la lógica, el contenido y la compatibilidad con el editor no
   cambian: es solo un cambio de diseño en el encabezado del grupo.

   Igual que antes: si el grupo tiene foto, el título va superpuesto sobre
   una banda; si no, se queda el encabezado sobrio (título en cursiva y
   línea fina). En ambos casos el título arranca en el carril de lectura. */
function pintarGrupo(grupo) {
  ...
  // La flecha que gira al desplegar, la misma idea que el filtro de
  // alérgenos: hace evidente que el encabezado se puede abrir y cerrar.
  const flecha = `<svg class="grupo__flecha" viewBox="0 0 24 24" fill="none" stroke="currentColor"
       stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
    <path d="m6 9 6 6 6-6"/>
  </svg>`;

  // La alerta va pegada al título: es una nota del grupo, así que se lee
  // junto a su nombre, como siempre.
  const conAlerta = alerta ? ' grupo__cabecera--con-alerta' : '';

  // El encabezado es ahora el <summary>: lo único visible con el grupo
  // plegado. Conserva título, alerta e imagen tal cual.
  const cabecera = (app.imagenes && grupo.imagen)
    ? `<summary class="grupo__cabecera grupo__cabecera--imagen${conAlerta}">
    ...
    : `<summary class="grupo__cabecera grupo__cabecera--simple columna${conAlerta}">
        ${titulo}
        ${alerta}
  ...
  // <details> sin el atributo "open": el grupo nace contraído.
  return `
    <details class="grupo">
      ${cabecera}
